...
function deadlineIndicator(deadline: string | null, daysLeft: number | null) {
  ...
  if (daysLeft <= 7) {
    return { label: `Urgent: ${daysLeft} day${daysLeft === 1 ? "" : "s"} left`, tone: "urgent" as const };
  }
  if (daysLeft <= 30) {
    return { label: `${daysLeft} day${daysLeft === 1 ? "" : "s"} left`, tone: "soon" as const };
  }
  return {
...
export function GrantCard({ grant, style, isLoggedIn = false }: GrantCardProps) {
    ...
    >
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {(grant.sector ?? []).map((s) => (
          <span
            key={s}
            className="rounded-full bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-900 ring-1 ring-emerald-600/10 transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:bg-emerald-500/[0.14]"
          >
            {s}
          </span>
        ))}
        {(grant.region ?? []).map((r) => (
          <span
            key={r}
            className="rounded-full bg-sky-500/10 px-2.5 py-1 text-xs font-medium text-sky-900 ring-1 ring-sky-500/15 transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:bg-sky-500/[0.14]"
          >
            {r}
          </span>
        ))}
        {(grant.eligibility ?? []).map((e) => (
          <span
            key={e}
            className="rounded-full bg-violet-500/10 px-2.5 py-1 text-xs font-medium text-violet-900 ring-1 ring-violet-500/15 transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] group-hover:bg-violet-500/[0.14]"
          >
            {e}
          </span>
        ))}
      ...
      <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-zinc-200/70 pt-3">
        {longDescription ? (
          <button
            ...
            className="text-xs font-semibold text-emerald-700 transition-colors duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] hover:text-emerald-800"
          >
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => {
              if (isLoggedIn) setSaved((v) => !v);
            }}
            disabled={!isLoggedIn}
            className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-semibold transition-colors duration-200 ease-out ${
              isLoggedIn
                ? saved
            ...
            title={isLoggedIn ? "Save this grant" : "Log in to save grants"}
          ...
          {normalized.applicationUrl && (
            <a
              ...
              className="inline-flex items-center gap-1 rounded-full bg-emerald-950 px-3 py-1.5 text-xs font-semibold text-white transition-colors duration-200 ease-out hover:bg-emerald-900"
            >
              Apply
